...
function buildFaviconDataUri(color) {
  const svg = `<svg width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="${FAVICON_PATH}" fill="${color}" stroke="${color}" stroke-width="0.753846"/></svg>`;
...
export default function DynamicFavicon() {
  ...
  const primaryColor = useMemo(() => {
    ...
    return selected?.swatches?.[1] || '#ef5087';
  }, [palette]);
  ...
  useEffect(() => {
    ...
    function applyFavicon() {
      const faviconHref = buildFaviconDataUri(primaryColor);
      const iconLink = ensureHeadLink('icon');
      const shortcutLink = ensureHeadLink('shortcut icon');
      const appleLink = ensureHeadLink('apple-touch-icon');
    ...
    // Reaplica após trocas de rota, já que o App Router pode recriar o head.
    applyFavicon();
    frame = window.requestAnimationFrame(applyFavicon);
    timeout = window.setTimeout(applyFavicon, 80);
