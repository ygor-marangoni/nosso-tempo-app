...
function setupListeners() {
  _cache.entries  = _lsGet(_LS.entries)  || [];
  _cache.config   = _lsGet(_LS.config)   || { name1: 'Ygor', name2: 'Julianne' };
  _cache.tags     = _lsGet(_LS.tags)     || [];
  _cache.album    = _lsGet(_LS.album)    || [];
  _cache.timeline = _lsGet(_LS.timeline) || [];
  _cache.phrases  = _lsGet(_LS.phrases)  || [];
...
// Operações de dados — salvam no localStorage e atualizam a UI
function addEntry(entry) {
  _cache.entries.push(entry);
  _lsSet(_LS.entries, _cache.entries);
  if (_appReady) _refreshCurrentPage();
}

function removeEntry(id) {
  _cache.entries = _cache.entries.filter(e => String(e.id) !== String(id));
  _lsSet(_LS.entries, _cache.entries);
  if (_appReady) _refreshCurrentPage();
}

function updateEntry(entry) {
  const idx = _cache.entries.findIndex(e => String(e.id) === String(entry.id));
  if (idx !== -1) _cache.entries[idx] = entry; else _cache.entries.push(entry);
  _lsSet(_LS.entries, _cache.entries);
  if (_appReady) _refreshCurrentPage();
}

function addPhotoToAlbum(photo) {
  _cache.album.push(photo);
  _lsSet(_LS.album, _cache.album);
  if (_appReady) _refreshCurrentPage();
}

function removePhotoFromAlbum(id) {
  _cache.album = _cache.album.filter(p => String(p.id) !== String(id));
  _lsSet(_LS.album, _cache.album);
  if (_appReady) _refreshCurrentPage();
}
...
function saveConf(c) {
  ...
  _lsSet(_LS.config, _cache.config);
  if (_appReady) _refreshCurrentPage();
}
...
function addMilestone(m) {
  _cache.timeline.push(m);
  _lsSet(_LS.timeline, _cache.timeline);
  if (_appReady) _refreshCurrentPage();
}
function removeMilestone(id) {
  _cache.timeline = _cache.timeline.filter(x => String(x.id) !== String(id));
  _lsSet(_LS.timeline, _cache.timeline);
  if (_appReady) _refreshCurrentPage();
}
function updateMilestone(m) {
  const idx = _cache.timeline.findIndex(x => String(x.id) === String(m.id));
  if (idx !== -1) _cache.timeline[idx] = m; else _cache.timeline.push(m);
  _lsSet(_LS.timeline, _cache.timeline);
  if (_appReady) _refreshCurrentPage();
}
...
function removeCouplePhoto() {
  showConfirm('Remover a foto do casal?', () => {
    const c = { ..._cache.config };
    delete c.couplePhoto;
    _cache.config = c;
    _lsSet(_LS.config, _cache.config);
    document.getElementById('cfg-photo-preview').style.display = 'none';
    document.getElementById('cfg-photo-placeholder').style.display = 'flex';
...
function addCouplePhrase() {
  ...
  if (!v) return;
  const id = Date.now();
  _cache.phrases.push({ id, text: v });
  _lsSet(_LS.phrases, _cache.phrases);
...
function removeCouplePhrase(id) {
  _cache.phrases = _cache.phrases.filter(p => String(p.id) !== String(id));
  _lsSet(_LS.phrases, _cache.phrases);
  renderPhrasesList();
}
...
function clearAllData() {
  showConfirm('Tem certeza que deseja apagar TODOS os dados?', () => {
    Object.values(_LS).forEach(k => localStorage.removeItem(k));
    _cache.entries = []; _cache.config = { name1: 'Ygor', name2: 'Julianne' };
    _cache.tags = []; _cache.album = []; _cache.timeline = []; _cache.phrases = [];
